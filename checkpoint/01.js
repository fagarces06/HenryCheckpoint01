...
function henryParty(arr) {
	...
	for(i in arr){
		if(!arr[i].ticket || !arr[i].estado) return false;
		if (arr[i].ticket==='VIP' && arr[i].estado==='Conocido') guest.enqueue(arr[i]);	
	}
...
var array=([{name:"Carol", ticket:"FALSO", estado: "Desconocido"},
 {name:"Gonzalo", ticket:"VIP", estado: "Conocido"}, 
